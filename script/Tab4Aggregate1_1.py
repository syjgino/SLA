...
def MergeApp(dirloc_aggregate, proname, method1loc, method2loc, maploc, CheckClustVis):
    ...
    if file2 != '':
        spequant2 = pd.read_excel(file2, sheet_name='Lipid Species Concentrations', header=0, index_col=0, na_values='.')
    ...
    for i in range(1,int(max(spequantin['ExpNum'])) + 1 ):
        ...
        specnormin_exp1PCA = specnormin_exp1PCA.drop(['ExpNum', 'GroupNum', 'SampleNorm', 'NormType', 'SampleID'], axis=0)
